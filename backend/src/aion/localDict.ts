...
export interface LocalPattern {
  ...
  examples: string[];
  response: string;
  toActions: (normalizedText: string, match: RegExpMatchArray) => AionAction[];
}
...
function extractPlugId(text: string): string {
...
function parsePercent(raw: string | undefined, defaultValue: number): number {
  if (!raw) {
    return defaultValue;
  }
...
function lightPowerAction(zone: string, power: 'on' | 'off'): AionAction {
  return { topic: `ovyon/control/lights/${zone}/power`, payload: power, qos: 1 };
}

function lightBrightnessAction(zone: string, brightness: number): AionAction {
  return { topic: `ovyon/control/lights/${zone}/brightness`, payload: String(brightness), qos: 1 };
}

function plugPowerAction(plugId: string, power: 'on' | 'off'): AionAction {
  return { topic: `ovyon/control/plugs/${plugId}/power`, payload: power, qos: 1 };
}

export const localPatterns: LocalPattern[] = [
  {
...
export const offlineCommandCatalog: string[] = [
  ...
  'open salon light',
  'turn on salon light',
  'switch on salon light',
  'turn off salon light',
  'switch off salon light',
  'turn on all lights',
  'turn off all lights',
