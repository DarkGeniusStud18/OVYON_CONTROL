...
export function handleMessage(topic: string, message: Buffer) {
  ...
  if (topic.startsWith('ovyon/control/')) {
    ...
    if (type === 'all' && action === 'power') {
      stateManager.updateAllDevices({ power: payload });
    }
  }
  ...
  if (topic.startsWith('ovyon/status/')) {
    ...
    let mappedId = '';
    if (type === 'lights') mappedId = `light_${id}`;
    else if (type === 'door') mappedId = `door_${id}`;
    else if (type === 'window') mappedId = `window_${id}`;
    else if (type === 'plugs') mappedId = `plug_${id}`;
    else if (type === 'sensor') mappedId = `sensor_${id}`;

    if (mappedId) {
        stateManager.updateDeviceState(mappedId, payload);
    }
  }
}
